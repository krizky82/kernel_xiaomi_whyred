...
# export KBUILD_COMPILER_STRING=$(/home/krizky/Kernel/dtc/out/7.0/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
...
make -j$(nproc --all) O=out \
                      ARCH=arm64 \
		      CC="/home/krizky/Kernel/dtc/out/7.0/bin/clang" \
